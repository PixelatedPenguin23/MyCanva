...
export default function CanvasAppV2() {
  ...
  useEffect(() => {
      ...
      tl.fromTo(
        introRef.current,
        { backgroundColor: '#111', filter: 'blur(20px)', scale: 1.5 },
        { backgroundColor: '#fff', filter: 'blur(0px)', scale: 0.9, duration: 2,ease: 'power2.out'  })

      tl.to(introRef.current,{scale:1,ease: 'power2.out',duration:0.5 })
}
    , [])
  // Canvas full screen
  useEffect(() => {
...
<label>
  ...
  <Button
    ...
    onClick={() => document.getElementById('shapeColorInput').click()}
  >
    Shape Color
  </Button>
</label>
